...
function dc_swp_str( $key ) {
    ...
    if ( $s === null ) {
        ...
        $s  = $da ? [
            ...
            'coi_desc'                => 'Sender <code>Cross-Origin-Opener-Policy: same-origin</code> og <code>Cross-Origin-Embedder-Policy: credentialless</code> på offentlige sider. Aktiverer <code>crossOriginIsolated</code> i browseren, så Partytown skifter til den hurtigere Atomics-bro i stedet for sync-XHR. Skip bots, indloggede brugere og kassen. Scripts og origins på ekskluderingslisten routes automatisk igennem en CORP-proxy (<code>/~partytown-corp-proxy</code>), der tilføjer <code>Cross-Origin-Resource-Policy: cross-origin</code>, så COEP ikke blokerer deres ressourcer. <strong>Test i staging — kan bryde OAuth-popups eller widgets med cross-origin iframes, der ikke er på ekskluderingslisten.</strong>',
            ...
            'coi_label'               => 'SharedArrayBuffer (Atomics Bridge)',
            'coi_desc'                => 'Sends <code>Cross-Origin-Opener-Policy: same-origin</code> and <code>Cross-Origin-Embedder-Policy: credentialless</code> on public pages. Enables <code>crossOriginIsolated</code> in the browser so Partytown switches to the faster Atomics bridge instead of the sync-XHR bridge. Skipped for bots, logged-in users and checkout. Scripts and origins on the exclusion list are automatically routed through a CORP proxy (<code>/~partytown-corp-proxy</code>) that adds <code>Cross-Origin-Resource-Policy: cross-origin</code>, preventing COEP from blocking their resources. <strong>Test in staging first — can break OAuth popups or widgets with cross-origin iframes that are not on the exclusion list.</strong>',
