...
The plugin slug is `dc-script-worker-prefetcher` but the code used the older
`dc-service-worker-prefetcher` text-domain in 193 places. All of those have
been corrected (193 → 0). The plugin header `Text Domain:` and the
`load_plugin_textdomain()` call now both use `dc-script-worker-prefetcher`,
and the language files have been renamed accordingly:
